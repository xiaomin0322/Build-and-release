...
final class NativeDate extends IdScriptableObject
{
    ...
    {
        NativeDate obj = new NativeDate(cx);
        // Set the value of the prototype Date to NaN ('invalid date');
        obj.date = ScriptRuntime.NaN;
        obj.exportAsJSClass(MAX_PROTOTYPE_ID, scope, sealed);
    }

    // JDK change: accept context and initialze timezone and localTZA
    // per context - rather than caching it in static fields.
    private NativeDate(Context cx)
    {
        cx.setTimeZone();
        cx.setLocalTZA();
    }
    ...
    private static double DaylightSavingTA(Context cx, double t)
    {
        ...
        if (!TZO_WORKAROUND) {
            ...
            if (cx.getTimeZone().inDaylightTime(date))
                return msPerHour;
            else
                return 0;
        } else {
            ...
            // happens at 2:00 AM:
            t += cx.getLocalTZA() + (HourFromTime(t) <= 2 ? msPerHour : 0);

            int year = YearFromTime(t);
            double offset = cx.getTimeZone().getOffset(year > 0 ? 1 : 0,
                                                   year,
                                                   MonthFromTime(t),
                                                   DateFromTime(t),
                                                   WeekDay(t),
    ...
    {
        return t + cx.getLocalTZA() + DaylightSavingTA(cx, t);
    }

    private static double internalUTC(Context cx, double t)
    {
        return t - cx.getLocalTZA() - DaylightSavingTA(cx, t - cx.getLocalTZA());
    }
    ...
    private static String date_format(Context cx, double t, int methodId)
    {
        ...
        if (methodId != Id_toDateString) {
            ...
            int minutes = (int) Math.floor((cx.getLocalTZA() + DaylightSavingTA(cx, t))
                                           / msPerMinute);
    ...
    private static Object jsConstructor(Context cx, Object[] args)
    {
        ...
        if (args.length == 1) {
            ...
            if (arg0 instanceof String) {
                ...
                date = date_parseString(cx, (String)arg0);
            } else {
