...
public class InterfaceAdapter
{
    ...
    static Object create(Context cx, Class<?> cl, Callable function)
    {
        ...
        // JDK change: using JavaAdapter instead of proxy generation here
        Scriptable topScope = ScriptRuntime.getTopCallScope(cx);
        Object v = ScriptableObject.getProperty(topScope, "JavaAdapter");
        if (v != Scriptable.NOT_FOUND) {
            Function javaAdapter = (Function) v;
            Scriptable obj = ScriptRuntime.newObject(cx, topScope,
                                         "Object", new Object[0]);
            for (int i = 0; i < methods.length; i++) {
                String name = methods[i].getName();
                ScriptableObject.putProperty(obj, name, function);
            }
            Object[] args = { cl, obj };
            Object res = javaAdapter.construct(cx, topScope, args);
